...
public class Stream_01_Test {
	...
	public void test_stream_filter_collect_counting() throws Exception {
		...
		long result = pizzas.stream().filter(pizza -> pizza.getPrice() >= 1300).toList().size();
	...
	public void test_stream_findFirst() throws Exception {
		...
		Optional<Order> result = orders.stream().filter(order -> order.getCustomer().getLastname().equals("Sophie")).findFirst();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		// pizza Pépéroni
		List<Order> result = orders.stream().filter(order -> order.getPizzas().stream().allMatch(pizza -> pizza.getName().equals("Pépéroni"))).toList();
